...
export default function TeamMember({ imageSrc, name, title, description, align }: TeamMemberProps) {
  ...
  return (
    <div className={`flex flex-col md:flex-row items-center bg-gray-800 p-8 rounded-lg shadow-lg max-w-4xl mx-auto my-24 ${isLeftAligned ? '' : 'md:flex-row-reverse'}`}>
      ...
      <div className={`text-center flex-grow ${isLeftAligned ? 'md:text-left' : 'md:text-right'}`}>
        ...
        <p className="text-gray-300 leading-relaxed" dangerouslySetInnerHTML={{ __html: description }}></p>
      </div>
    </div>
  );
}
